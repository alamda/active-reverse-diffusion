...
with open(test_fname, 'r+b') as f:
    ...
    mm_arr = torch.from_numpy(np.reshape(mm_arr, arr_shape))
...
for num in elements_to_load:
    ...
    with open(test_fname, 'r+b') as f:
        ...
        mm_tensor[:num] = torch.from_numpy(np.zeros(new_tensor.shape))
