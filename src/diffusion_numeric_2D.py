...
class DiffusionNumeric2D(Diffusion2D):
    ...
    def compute_loss_active(self, t_idx,
        ...
        if len(Fx.shape) == 1:
            Fx = Fx.reshape((Fx.shape[0], 2))
        if len(Feta.shape) == 1:
            Feta = Feta.reshape((Feta.shape[0], 2))
        if len(x.shape) == 1:
            x = x.reshape((x.shape[0], 2))
        if len(eta.shape) == 1:
            eta = eta.reshape((eta.shape[0], 2))
    ...
    def train_diffusion_active(self, nrnodes=4, iterations=500):
        ...
        for e in bar:
            score_model_x = torch.nn.Sequential(
                torch.nn.Linear(4, nrnodes), torch.nn.Tanh(),
                torch.nn.Linear(nrnodes, nrnodes), torch.nn.Tanh(),
                torch.nn.Linear(nrnodes, nrnodes), torch.nn.Tanh(),
                torch.nn.Linear(nrnodes, 2)
            ).double()
