...
def do_diffusion(data, tsteps, T, dt):
    ...
    for t in range(tsteps):
        xt = xt - dt*xt + np.sqrt(2*T*dt)*torch.normal(torch.zeros_like(data), torch.ones_like(data))
        samples.append(xt)
    return samples

def compute_loss(forward_samples, T, t, dt, score_model):
    xt = forward_samples[t]         # x(t)
    l = -(xt - forward_samples[0]*np.exp(-t*dt))/(T*(1-np.exp(-2*t*dt))) # The actual score function
    scr = score_model(xt)
...
def passive_training(dataset, tsteps, T, dt, nrnodes=4, iterations=500):
    ...
    for e in bar:
        ...
        optim = torch.optim.AdamW(itertools.chain(score_model.parameters()), lr=1e-2, weight_decay=1e-8,)
        loss = 100
...
def sampling(N, all_models, T, dt, tsteps):
    ...
    for t in range(tsteps-2, 0, -1):
        F = all_models[t](x);
        x = x + x*dt + 2*T*F*dt + np.sqrt(2*T*dt)*torch.randn(N,1)   # If using the total score function 
...
def do_diffusion_active(data, tsteps, Tp, Ta, tau, dt):
    eta = torch.normal(torch.zeros_like(data), np.sqrt(Ta/tau)*torch.ones_like(data))
    samples = [data]
    eta_samples = [eta]
    xt = data
    for t in range(tsteps):
        xt = xt - dt*xt + dt*eta + np.sqrt(2*Tp*dt)*torch.normal(torch.zeros_like(data), torch.ones_like(data))
        eta = eta - (1/tau)*dt*eta + (1/tau)*np.sqrt(2*Ta*dt)*torch.normal(torch.zeros_like(eta), torch.ones_like(eta))
        samples.append(xt)
...
def active_training(dataset, tsteps, Tp, Ta, tau, k, dt, nrnodes=4, iterations=500):
    ...
    all_models_x = []
    all_models_eta = []
    forward_samples, forward_samples_eta = do_diffusion_active(dataset, tsteps, Tp, Ta, tau, dt)
    t = 1
    bar = tqdm(range(1, tsteps))
    for e in bar:
        ...
        optim_x = torch.optim.AdamW(itertools.chain(score_model_x.parameters()), lr=1e-2, weight_decay=1e-8,)
        optim_eta = torch.optim.AdamW(itertools.chain(score_model_eta.parameters()), lr=1e-2, weight_decay=1e-8,)
        loss = 100
        for _ in range(iterations):
            optim_x.zero_grad()
            optim_eta.zero_grad()
            loss_x, loss_eta, loss_Fx, loss_Feta, loss_scr_x, loss_scr_eta = score_function_loss(forward_samples, forward_samples_eta, Tp, Ta, tau, k, t, dt, score_model_x, score_model_eta)
            loss_x.backward()
            loss_eta.backward()
            optim_x.step()
            optim_eta.step()
        bar.set_description(f'Time:{t} Loss: {loss_x.item():.4f} Fx: {loss_Fx.item():.4f} scr_x: {loss_scr_x.item():.4f}')
...
def sampling_active(N, all_models_x, all_models_eta, Tp, Ta, tau, k, dt, tsteps):
    x = np.sqrt(Tp/k + (Ta/(k*k*tau+k)))*torch.randn([N,1])
    eta = np.sqrt(Ta/tau)*torch.randn([N,1])
    samples_x = [x.detach()]
    ...
    for t in range(tsteps-2, 0, -1):
        ...
        eta = eta + dt*eta/tau + (2*Ta/(tau*tau))*Feta*dt + (1/tau)*np.sqrt(2*Ta*dt)*torch.randn(eta.shape)
