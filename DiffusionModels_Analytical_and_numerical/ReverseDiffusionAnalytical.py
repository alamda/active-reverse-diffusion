...
def reverse_process_passive_new_multiple(x, T, tsteps, dt, sigmalist, mulist, plist):
    ...
    for t in range(tsteps, 0, -1):
        Fx = score_function_passive_new_multiple(x, T, mulist, sigmalist, plist, t*dt)
...
def score_function_new_multiple(x, eta, Tp, Ta, tau, mulist, sigmalist, plist, k, t):
    ...
    for i in range(len(mulist)):
        ...
        K2 = b*c*g + M12
        K3 = b*b*g + a*a*h + M11
        Delta_eff = c*c*g*M11 - 2*b*c*g*M12 - M12*M12 + b*b*g*M22 + M11*M22 + a*a*h*(c*c*g + M22)
        z = np.exp((-K1*(x-a*mu)**2 + 2*K2*(x-a*mu)*eta - K3*eta**2)/(2*Delta_eff))
        Fx_num = Fx_num + p*h*np.power(Delta_eff, -1.5)*(K2*eta - K1*(x-a*mu))*z
        Fx_den = Fx_den + p*h*np.power(Delta_eff, -0.5)*z
        Feta_num = Feta_num + p*h*np.power(Delta_eff, -1.5)*(K2*(x-a*mu) - K3*eta)*z
...
def reverse_process_active_new_multiple(x, y, Tp, Ta, tau, tsteps, dt, sigmalist, mulist, plist, k):
    ...
    ymat[:,tsteps] = y
    for t in range(tsteps, 0, -1):
        Fx, Fy = score_function_new_multiple(x, y, Tp, Ta, tau, mulist, sigmalist, plist, k, t*dt)
        x = x + dt*(x-y) + 2*Tp*Fx*dt + np.sqrt(2*Tp*dt)*np.random.randn(len(x))
        y = y + dt*y/tau + (2*Ta/(tau*tau))*Fy*dt + (1/tau)*np.sqrt(2*Ta*dt)*np.random.randn(len(y))
        xmat[:,t-1] = x
        ymat[:,t-1] = y
    return xmat, ymat
